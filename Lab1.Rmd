...
```{r 2.7, warning=FALSE}
 ...
 q1 <- quantile(SENIC$V3,probs = 0.25)
  q3 <- quantile(SENIC$V3,probs = 0.75)
  below_value <- SENIC$V3 < q1 - 1.5 * (q3-q1)
  upper_value <- SENIC$V3> q3 + 1.5 * (q3 -q1)
  outliers <- ifelse(below_value | upper_value , TRUE , FALSE)
  

graph5 <- SENIC%>% 
  plot_ly( x = ~infection_rate,name = "infection rate") %>%
  add_histogram(title = "histogram of infection rate")%>%
  add_markers(data =  SENIC[assignment2(SENIC$V3),],y=0,name = "outliers")%>%
