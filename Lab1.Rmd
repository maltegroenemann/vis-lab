...
```{r 2.5}
ggplot(data=SENIC,aes(x=infection_risk,y=n_nurses,color = n_beds))+
           geom_point()+
            labs(x = "Infection risk", y= "Number of nurses")
...
```{r 2.7}
plot_ly(senic_data, x = ~infection_risk,fill = ~n_nurses) %>%
  add_histogram()%>%
 q  
```
