...
>In plot 1.2 , human pre ateentive mechanism can identify the bright hue in plot ,despite of over plotting the points can be differentiated easily.
...
>George miller measured the levels of channel capacities in bits for each hues, shapes and lengths, channel capacities are measured in levels or  bits.
...
>According to channel capacities for *Size of squares* experiment , humans were able to classify only just 4 to 5 levels, this suggests that it is difficult for humans to identify sizes correctly , In plot 1.4 this can be clearly noticed, over plotting of points can be  clearly observed and the use of single hue for points can lead to wrong perception,2.2 bits can be encoded for this aesthetic.
 
>Channel capacities for *Line geometry* experiment by pollack suggests, humans were able to classify only just 2.8 - 3.3 bits,this suggests that it is difficult for humans to identify orientation angle, In plot 1.5 this can be clearly noticed , 3 bits can be encoded in plot 1.5. 

>Among the three plots ,plot 1.4 is hard to differentiate between the categories, as least number of bits can be encoded among other two plots.
...
>In both plots, the only channel capacity feature is hues and brightness, it is difficult for human vision to distinguish between the hue variances of the points in plot 1.6 with a pre-attention mechanism, even with an attentive mechanism, it is not possible to differentiate between the points.Over plotting of points is observed for two categories of points ,this may lead to slow identification of decision boundaries.


>Boundaries between two  regions overlapping each other cannot be determined with pre attentive mechanism
...
>In the plot 1.8 , the points were categorized using three different features "color" ,"shape" , "size" using "linoleic" , "palmitic" and "palmitoleic" variables respectively. 

>Use of various channel capacites together is also known as *Multi-dimensional stimuli*.

>*pollack/ficks* while researching on *multi-dimensional stimuli* , they found out that even when using multiple features ,maximum of 7.2 were encoded,adding multiple stimuli reduced the discernibility of individual feature.

>Along with channel capacities there were few other issues found while using multiple aesthetics. The plot has over plotting which intern makes it difficult to see overlapping data points and it has too many classes to understand,due to this perception of data is confusing and this can lead wrong data interpretation.
...
>In the plot 1.9 boundaries can be found up to a limited extent, we can observe an boundary between two regions at bottom as both regions have similar aesthetic and color difference can be observed.

>According to treisman's theory of pre-attentive processing , conjuction of features require serial search between the maps.
...
>In next layer we can distinguish shape of the points , two regions at bottom has higher number of tringles, the central cluster consists squares and triangles , but due to overplotting much specifications cannot be determined.
...
>Good principles of visualization suggests a plot should possess a proper legend ,a 100 percent scale and information.
...
>If we look at the scatterplot, there are some points between 800 and 1000, but the 2d density reveals that the regions is very empty.


>In the density plot, the plot did not define the outliers of the givn data, without the outlier information some key information might be missed.
